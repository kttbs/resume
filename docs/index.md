...
- アジャイル、スクラムの推進
- 各AI Agent使用時の開発効率化のためのSkillsチューニング
...
toC, toB向けプロダクトの機能開発として参画。のちに併せてAI推進チームにも参画し、AI Agentを用いた開発業務効率化のためのSkills構築、布教も担当。
機能開発においては新規機能の設計レビューおよび実装を担当しタスクのSPより前倒しで完了するよう努める。
併せて、レガシーコードを現ドメイン構造を含むクリーンアーキテクチャに準拠するように修正するなどのリファクタリングも進める。
この現場におけるtoBのプロダクトでは、ドメインマスターと言える立場の設計者が作成するほぼ完成された仕様書に準拠した実装を進めるという流れだったため、
AgentSkillsを社員の方とペアプロしながら構築し、品質のおよび開発効率の向上に努めた。
toCのプロダクトではドメインおよびデータモデルの設計レビュー/実装、一部Ruby on Railsで実装されている管理画面も担当。
...
- **使用技術：**
	...
	- AI Agent
		- Codex (EnterPrise)
		- Claude Code (Bedrock経由)
- **役割：**
	- 設計レビュー、バックエンド/(フロントエンド)開発、 AI Agent利用におけるSkills構築、布教
- **担当プロジェクト：**
	- toBプロダクトのバックエンド開発
		- 仕様書に準拠した実装、および仕様不備に対するフィードバック
		- レガシーコード(弱いDDD)を、現ドメイン構造に準拠するように修正するなどのリファクタリング
	- toCプロダクトの設計レビュー/バックエンド開発
		- ドメインおよびデータモデルの設計レビュー/実装
		- Rubyで構築されている管理画面の実装
	- その他
		- AI Agentを用いた開発業務効率化のためのSkills構築、布教
			- CodexおよびClaudeCodeを用いた、コード生成のためのプロンプトエンジニアリングの試行錯誤、生成されたコードに対するチームへのフィードバック
...
- AI Agentを用いた開発業務効率化のためのSkills/Hooksの設定を含むチューニングに興味があります
